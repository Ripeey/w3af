...
class _fileobject(object):
    ...
    def read(self, size=-1):
        ...
        if size < 0:
            # Read until EOF
            self._rbuf = io.BytesIO()  # reset _rbuf.  we consume it via buf.
            while True:
                try:
...
class SSLSocket(object):
    ...
    # patchFIX makefile buff=0 as default in py2.7 but not from py3.9
    def makefile(self, mode, bufsize=0):
        ...
        """
        self.close_refcount += 1
        # patchFIX makefile _fileobject doesnt exist either switch to socketIO or just makefile
        # also it seems like underlying socket doesnt close anymore, might need .close here
        # nvm for now just cherry picked whole class here, a better solution would be not use the wapper at all (BFD error [if] can be other method)
...
def wrap_socket(sock, keyfile=None, certfile=None, server_side=False,
    ...
    if server_hostname is not None:
        cnx.set_tlsext_host_name(smart_bytes(server_hostname))
